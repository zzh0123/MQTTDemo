...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        builder.target(new LatLng(31.84408,117.143266)); // 31.84408, 117.143266
        builder.zoom(19.0f);
        ...
        OverlayOptions markerOptions = new MarkerOptions().flat(true).anchor(0.5f, 0.5f).icon(mBitmapCar).
                position(latlngs[0]).rotate((float) getAngle(0));
    ...
    public void moveLooper(final Marker marker) {
        new Thread() {
            public void run() {
                while (true) {
                    for (int i = 0; i < latlngs1.length - 1; i++) {
                        final LatLng startPoint = latlngs1[i];
                        final LatLng endPoint = latlngs1[i + 1];
...
//    };
    ...
    private static final LatLng[] latlngs1 = new LatLng[]{
